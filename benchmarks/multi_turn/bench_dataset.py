...
class LognormalDistribution(Distribution):
    def __init__(
        ...
        if average is not None:
            ...
            mean, sigma = self._generate_lognormal_by_median(
                target_average=self.average, median_ratio=median_ratio
            )
        else:
    ...
    def _generate_lognormal_by_median(
        ...
        * median(X) = exp(mu)
        ...
        median_target = target_average * median_ratio

        # Solve sigma^2 = 2 * ln(mean / median)
        sigma = np.sqrt(2 * np.log(target_average / median_target))
    ...
    def __repr__(self) -> str:
        if self.average:
            return f"LognormalDistribution[{self.average}, {self.max_val}]"
...
def get_random_distribution(
    ...
    if distribution == "constant":
        ...
        if "average" in conf:
            ...
            return LognormalDistribution(average=conf["average"],
